...
'changequote`'')
changecom()dnl
syscmd(`hash check_version 2> /dev/null')
ifelse(sysval, `0', `', `
errprint(__program__:__file__:__line__: `ERROR: cannot continue without "check_version" in PATH.')
m4exit(sysval)
')
...
bind ifelse(HAVE_NOTE, `yes', `-N "copy into X clipboard"') \
	-T copy-mode-vi y if-shell "hash xclip" "send -X copy-pipe RQ()xclip -i`'RQ()" "send -X copy-selection"
...
syscmd(`hash pidcmd 2> /dev/null')
define(`HAVE_PIDCMD', ifelse(sysval, `0', `yes', `no'))
...
# `HAVE_PIDCMD': HAVE_PIDCMD
ifelse(FANCY_FORMAT, `yes', `
ifelse(HAVE_PIDCMD, `yes', ` dnl format with pidcmd
